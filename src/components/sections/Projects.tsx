...
/**
 * Projects section: showcases selected work as a responsive card grid.
 */
    ...
    >
      {projects.map((project) => (
        <article
          ...
          aria-labelledby={getProjectHeadingId(project.title)}
        >
          <div>
            <h3
              id={getProjectHeadingId(project.title)}
              className="text-sm sm:text-base font-semibold text-slate-50 mb-2"
            >
              {project.title}
            </h3>
...
function getProjectHeadingId(title: string): string {
  return `project-${title.replace(/\s+/g, "-").toLowerCase()}-heading`;
}
